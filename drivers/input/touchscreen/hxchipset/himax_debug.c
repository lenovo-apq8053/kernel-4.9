...
static int16_t diag_self[100] = {0};
	...
	size_t len, loff_t *pos)
{
	...
	himax_int_enable(private_ts->client->irq, 1);
		...
		// open file
		ret = request_firmware(&fw, fileName, private_ts->dev);
		if (ret) {
		...
		if ((buf[1] == '6') && (buf[2] == '0')) {
			if (fts_ctpm_fw_upgrade_with_sys_fs_60k(private_ts->client, fw_data, fw->size, false) == 0) {
			...
			if (fts_ctpm_fw_upgrade_with_sys_fs_64k(private_ts->client, fw_data, fw->size, false) == 0) {
				E("%s: TP upgrade error, line: %d\n", __func__, __LINE__);
				fw_update_complete = false;
			} else {
				I("%s: TP upgrade OK, line: %d\n", __func__, __LINE__);
			...
			if (fts_ctpm_fw_upgrade_with_sys_fs_124k(private_ts->client, fw_data, fw->size, false) == 0) {
				E("%s: TP upgrade error, line: %d\n", __func__, __LINE__);
				fw_update_complete = false;
			} else {
				I("%s: TP upgrade OK, line: %d\n", __func__, __LINE__);
			...
			if (fts_ctpm_fw_upgrade_with_sys_fs_128k(private_ts->client, fw_data, fw->size, false) == 0) {
				E("%s: TP upgrade error, line: %d\n", __func__, __LINE__);
				fw_update_complete = false;
			} else {
				I("%s: TP upgrade OK, line: %d\n", __func__, __LINE__);
...
int himax_touch_proc_init(void)
{
	...
	himax_proc_debug_level_file = proc_create(HIMAX_PROC_DEBUG_LEVEL_FILE, (0644), himax_touch_proc_dir, &himax_proc_debug_level_ops);
	if (himax_proc_debug_level_file == NULL) {
		E(" %s: proc debug_level file create failed!\n", __func__);
		goto fail_1;
	}

	himax_proc_vendor_file = proc_create(HIMAX_PROC_VENDOR_FILE, (0444), himax_touch_proc_dir, &himax_proc_vendor_ops);
	...
	himax_proc_int_en_file = proc_create(HIMAX_PROC_INT_EN_FILE, (0644), himax_touch_proc_dir, &himax_proc_int_en_ops);
	if (himax_proc_int_en_file == NULL) {
		E(" %s: proc int en file create failed!\n", __func__);
		goto fail_4;
	}

	himax_proc_layout_file = proc_create(HIMAX_PROC_LAYOUT_FILE, (0644), himax_touch_proc_dir, &himax_proc_layout_ops);
...
#ifdef HX_TP_PROC_RESET
	himax_proc_reset_file = proc_create(HIMAX_PROC_RESET_FILE, (0200), himax_touch_proc_dir, &himax_proc_reset_ops);
...
#ifdef HX_TP_PROC_DIAG
	himax_proc_diag_file = proc_create(HIMAX_PROC_DIAG_FILE, (0644), himax_touch_proc_dir, &himax_proc_diag_ops);
	if (himax_proc_diag_file == NULL) {
		E(" %s: proc diag file create failed!\n", __func__);
		goto fail_7;
	}
	himax_proc_diag_arrange_file = proc_create(HIMAX_PROC_DIAG_ARR_FILE, (0644), himax_touch_proc_dir, &himax_proc_diag_arrange_ops);
...
#ifdef HX_TP_PROC_REGISTER
	himax_proc_register_file = proc_create(HIMAX_PROC_REGISTER_FILE, (0644), himax_touch_proc_dir, &himax_proc_register_ops);
...
#ifdef HX_TP_PROC_DEBUG
	himax_proc_debug_file = proc_create(HIMAX_PROC_DEBUG_FILE, (0644), himax_touch_proc_dir, &himax_proc_debug_ops);
...
#ifdef HX_HIGH_SENSE
	himax_proc_HSEN_file = proc_create(HIMAX_PROC_HSEN_FILE, (0666), himax_touch_proc_dir, &himax_proc_HSEN_ops);
...
#ifdef HX_SMART_WAKEUP
	himax_proc_SMWP_file = proc_create(HIMAX_PROC_SMWP_FILE, (0666), himax_touch_proc_dir, &himax_proc_SMWP_ops);
	if (himax_proc_SMWP_file == NULL) {
		E(" %s: proc SMWP file create failed!\n", __func__);
		goto fail_13;
	}
	himax_proc_GESTURE_file = proc_create(HIMAX_PROC_GESTURE_FILE, (0666), himax_touch_proc_dir, &himax_proc_Gesture_ops);
...
#ifdef HX_TP_PROC_SENSE_ON_OFF
	himax_proc_SENSE_ON_OFF_file = proc_create(HIMAX_PROC_SENSE_ON_OFF_FILE, (0666), himax_touch_proc_dir, &himax_proc_sense_on_off_ops);
