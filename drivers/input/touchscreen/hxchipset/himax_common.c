...
static void HX_report_ESD_event(void);
static unsigned char ESD_00_counter;
static unsigned char ESD_00_Flag;
#endif
...
static int touch_monitor_stop_flag;
#endif
...
static void calcDataSize(uint8_t finger_num)
{
	...
	I("%s: coord_data_size: %d, area_data_size:%d", __func__,
		ts_data->coord_data_size, ts_data->area_data_size);
	I("%s: raw_data_frame_size:%d, raw_data_nframes:%d", __func__,
...
//	uint8_t tmp_data[4];
	...
	i_FW_VER = i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] << 8
			| i_CTPM_FW[FW_VER_MIN_FLASH_ADDR];
	i_CFG_VER = i_CTPM_FW[CFG_VER_MAJ_FLASH_ADDR] << 8
	...
	CRC_Check_result = Calculate_CRC_with_AP(ImageBuffer,
	...
	if ((CRC_Check_result == 0) || (ic_data->vendor_fw_ver < i_FW_VER)
...
HSEN_bit_retry:
		...
		I("%s: Read HSEN data[0]=%x data[1]=%x data[2]=%x data[3]=%x\n",
...
static int himax_parse_wake_event(struct himax_ts_data *ts)
{
	...
	int tmp_max_y = 0x00, tmp_min_y = 0xFFFF;
	int gest_len;
#endif
	...
	for (i = 0; i < GEST_PTLG_ID_LEN; i++) {
		if (check_FC == 0) {
			...
			if (buf[i] != gesture_flag) {
				check_FC = 0;
				I("ID NOT the same %x != %x. STOP parse event\n",
							buf[i], gesture_flag);
				break;
			}
		}
...
#endif
#ifdef HX_SMART_WAKEUP
	queue_delayed_work(ts->himax_smwp_wq, &ts->smwp_work,
				msecs_to_jiffies(1000));
#endif
#ifdef HX_HIGH_SENSE
	queue_delayed_work(ts->himax_hsen_wq, &ts->hsen_work,
				msecs_to_jiffies(1000));
#endif
	return 0;
