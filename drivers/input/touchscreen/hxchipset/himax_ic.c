...
int himax_hand_shaking(struct i2c_client *client)
{
	...
	ret = i2c_himax_read(client, 0xD1, hw_reset_check,
	...
	if (hw_reset_check[0] != IC_STATUS_CHECK) {
		usleep_range(2000, 4000);
		ret = i2c_himax_read(client, 0xD1, hw_reset_check_2,
						1, HIMAX_I2C_RETRY_TIMES);
		if (ret < 0) {
			E("[Himax]:i2c_himax_read 0xD1 failed: %d\n", __LINE__);
			goto work_func_send_i2c_msg_fail;
		}
...
int himax_chip_self_test(struct i2c_client *client)
{
	...
	for (j = 0; j < 10; j++) {
		...
		if (tmp_data[0] != 0) {
			...
			if (i2c_himax_write(client, 0x00, tmp_data,
						4, HIMAX_I2C_RETRY_TIMES) < 0) {
				E("%s: i2c access fail!\n", __func__);
			}

			tmp_data[0] = 0x00;
			if (i2c_himax_write(client, 0x0C, tmp_data,
						1, HIMAX_I2C_RETRY_TIMES) < 0) {
				E("%s: i2c access fail!\n", __func__);
			}
			i2c_himax_read(client, 0x08, tmp_data,
					124, HIMAX_I2C_RETRY_TIMES);
		} else {
			break;
		}
	}
...
int himax_burst_enable(struct i2c_client *client, uint8_t auto_add_4_byte)
{
	...
	tmp_data[0] = 0x31;
	if (i2c_himax_write(client, 0x13, tmp_data,
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return -EBUSY;
	}
	...
	if (i2c_himax_write(client, 0x0D, tmp_data,
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return -EBUSY;
	}
				...
				int read_length, uint8_t *read_data)
{
	...
	if (i2c_himax_read(client, 0x08, read_data, read_length * 4,
		...
		uint8_t *reg_byte, uint8_t *read_data)
{
	...
	if (i2c_himax_write(client, 0x00, &reg_byte[0],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}

	if (i2c_himax_write(client, 0x01, &reg_byte[1],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}

	if (i2c_himax_write(client, 0x02, &reg_byte[2],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}

	if (i2c_himax_write(client, 0x03, &reg_byte[3],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
	...
	tmpbyte[0] = 0x00;
	if (i2c_himax_write(client, 0x0C, &tmpbyte[0],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}

	if (i2c_himax_read(client, 0x08, &read_data[0],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}

	if (i2c_himax_read(client, 0x09, &read_data[1],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}

	if (i2c_himax_read(client, 0x0A, &read_data[2],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}

	if (i2c_himax_read(client, 0x0B, &read_data[3],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}

	if (i2c_himax_read(client, 0x18, &tmpbyte[0],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	} // No bus request

	if (i2c_himax_read(client, 0x0F, &tmpbyte[1],
				1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	} // idle state
			...
			uint8_t *reg_byte, uint8_t *write_data)
{
	...
	if (i2c_himax_write(client, 0x00, data_byte,
				8, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
			...
			uint8_t *reg_byte, uint8_t *write_data, int length)
{
	...
	if (i2c_himax_write(client, 0x00, data_byte,
				length + 4, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return -EBUSY;
	}
...
int himax_register_write(struct i2c_client *client, uint8_t *write_addr,
			int write_length, uint8_t *write_data)
{
	...
	for (i = address; i < address + write_length * 4; i = i + 4) {
		...
		ret = himax_flash_write_burst_length(client, write_addr,
					write_data, write_length * 4);
		if (ret < 0)
			return ret;
	}
...
void himax_sense_off(struct i2c_client *client)
{
	...
	himax_register_read(client, tmp_addr, 1, tmp_data);
	I("%s: CPU clock off password\n", __func__);
	I("%s: data[0]=%x data[1]=%x data[2]=%x data[3]=%x\n", __func__,
...
bool wait_wip(struct i2c_client *client, int Timing)
{
	...
	do {
		...
		if (in_buffer[0] != 0x00 || in_buffer[1] != 0x00
			...
			I("%s:Wait wip retry_cnt:%d\n", __func__, retry_cnt);
			I("%s:buffer[0]=%d, buffer[1]=%d\n", __func__,
					in_buffer[0], in_buffer[1]);
			I("%s:buffer[2]=%d, buffer[3]=%d\n", __func__,
...
void himax_sense_on(struct i2c_client *client, uint8_t FlashMode)
{
	...
	I("%s: check pull low CPU reset signal\n", __func__);
	I("%s: data[0]=%x data[1]=%x data[2]=%x data[3]=%x\n", __func__,
			tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
	...
	I("%s: revert pull low CPU reset signal\n", __func__);
	I("%s: data[0]=%x data[1]=%x data[2]=%x data[3]=%x\n", __func__,
			tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
				...
				uint8_t *FW_content, int FW_Size)
{
	...
	for (page_prog_start = 0; page_prog_start < FW_Size;
		...
		tmp_data[1] = 0xF0;
		tmp_data[0] = 0x00;
		// data bytes should be
		// 0x6100_0000 + ((word_number)*4-1)*4096 =
...
bool himax_check_chip_version(struct i2c_client *client)
{
	...
	// 4. After read finish, set DDREG_Req = 0 (0x9000_0020 = 0x0000_0000)
	//    (Unlock register R/W from driver)
	tmp_addr[3] = 0x90;
	tmp_addr[2] = 0x00;
	tmp_addr[1] = 0x00;
	tmp_addr[0] = 0x20;
...
void himax_touch_information(struct i2c_client *client)
{
	...
	if (IC_TYPE == HX_83100_SERIES_PWON) {
		...
		I("%s:HX_XY_REVERSE =%d,HX_Y_RES =%d,HX_X_RES=%d\n", __func__,
				ic_data->HX_XY_REVERSE, ic_data->HX_Y_RES,
				ic_data->HX_X_RES);
...
bool himax_ic_package_check(struct i2c_client *client)
{
	...
	FW_VER_MAJ_FLASH_ADDR  = 57384;
	//0xE028
	FW_VER_MAJ_FLASH_LENG  = 1;
	FW_VER_MIN_FLASH_ADDR  = 57385;
	//0xE029
	FW_VER_MIN_FLASH_LENG  = 1;
	CFG_VER_MAJ_FLASH_ADDR = 58115;
	//0xE303
	CFG_VER_MAJ_FLASH_LENG = 1;
	CFG_VER_MIN_FLASH_ADDR = 58116;
	//0xE304
...
bool diag_check_sum(uint8_t hx_touch_info_size, uint8_t *buf)
{
	...
	if (check_sum_cal % 0x10000 != 0) {
		I("%s: diag check sum fail!\n", __func__);
		I("%s: check_sum_cal=%X, hx_touch_info_size=%d,\n", __func__,
				check_sum_cal, hx_touch_info_size);
		return 0;
	}
	return 1;
}
...
void diag_parse_raw_data(int hx_touch_info_size, int RawDataLen, int mul_num,
		int self_num, uint8_t *buf, uint8_t diag_cmd,
	...
	if (buf[hx_touch_info_size] == 0x3A
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) { //mutual
				...
				mutual_data[index + i] =
					buf[i*2 + hx_touch_info_size+4+1]*256
